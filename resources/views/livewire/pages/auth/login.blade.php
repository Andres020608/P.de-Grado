...
<div class="relative min-h-screen overflow-hidden">
    ...
    <main class="relative z-10 mx-auto flex min-h-screen w-full max-w-[1200px] flex-col items-center justify-center gap-16 px-6 py-12">
        <div class="hidden md:flex max-w-md flex-col">
            ...
            <p class="text-lg text-[#404945] leading-relaxed mb-8 opacity-90">Ingresa al atelier digital y accede a una colección curada de piezas atemporales hechas para el conocedor moderno.</p>
        ...
        <div class="w-full max-w-[480px] rounded-[32px] border border-white/20 p-8 md:p-12 glass-panel shadow-[0_20px_40px_rgba(27,28,25,0.05)]">
            <div class="text-center mb-10">
                ...
                <h2 class="text-2xl font-bold text-[#003229] tracking-tight" style="font-family: 'Noto Serif', serif;">Bienvenido de nuevo</h2>
                <p class="text-sm text-[#404945] mt-2">Inicia sesión en tu colección privada</p>
            </div>

            <div class="flex border-b border-[#bfc9c4]/30 mb-8">
                <a href="{{ route('login') }}" class="flex-1 pb-4 font-semibold text-xs uppercase tracking-[0.2em] text-[#735c00] border-b-2 border-[#735c00]">Ingresar</a>
                <a href="{{ route('register') }}" class="flex-1 pb-4 font-semibold text-xs uppercase tracking-[0.2em] text-[#404945]/60 hover:text-[#003229] transition">Registrarse</a>
            </div>

            <x-auth-session-status class="mb-4 text-sm text-[#003229]" :status="session('status')" />

            <form wire:submit="login" class="space-y-6">
                <div>
                    <label for="email" class="block text-[10px] uppercase tracking-[0.2em] text-[#404945] mb-1">Correo electrónico</label>
                    <input wire:model="form.email" id="email" name="email" type="email" required autofocus autocomplete="username" placeholder="atelier@imperial.com" class="w-full bg-transparent border-0 border-b border-[#bfc9c4] py-3 px-0 pl-1 text-sm text-[#1b1c19] focus:outline-none focus:border-[#735c00] transition" />
                    <x-input-error :messages="$errors->get('form.email')" class="mt-2 text-xs text-[#ba1a1a]" />
                </div>

                <div>
                    <div class="flex justify-between items-end mb-1">
                        <label for="password" class="text-[10px] uppercase tracking-[0.2em] text-[#404945]">Clave de seguridad</label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" wire:navigate class="text-[10px] uppercase tracking-[0.2em] text-[#735c00] hover:underline">¿Olvidaste tu clave?</a>
                        @endif
                    </div>
                    <div class="relative">
                        <input wire:model="form.password" id="password" name="password" type="password" required autocomplete="current-password" placeholder="••••••••••••" class="w-full bg-transparent border-0 border-b border-[#bfc9c4] py-3 px-0 pl-1 text-sm text-[#1b1c19] focus:outline-none focus:border-[#735c00] transition" />
                        <button type="button" onclick="togglePasswordVisibility('password', this)" class="absolute right-0 bottom-3 text-[#404945]/60 hover:text-[#003229] transition">
                            <span class="material-symbols-outlined text-sm">visibility</span>
                        </button>
                    </div>
                    <x-input-error :messages="$errors->get('form.password')" class="mt-2 text-xs text-[#ba1a1a]" />
                </div>

                <div class="flex items-center gap-3 pt-2">
                    <input wire:model="form.remember" id="remember" type="checkbox" class="w-4 h-4 rounded-sm border-[#bfc9c4] text-[#003229] focus:ring-[#003229]" />
